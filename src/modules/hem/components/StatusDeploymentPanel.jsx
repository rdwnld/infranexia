...
const STAGE_COLORS = {
  [STAGES.APPROVED_DROP]: { border: 'border-rose-500/50', bg: 'bg-rose-950/40', text: 'text-rose-400' },
  [STAGES.PROPOSED_DROP]: { border: 'border-orange-500/50', bg: 'bg-orange-950/40', text: 'text-orange-400' },
  [STAGES.PERSIAPAN]: { border: 'border-amber-500/50', bg: 'bg-amber-950/40', text: 'text-amber-400' },
  [STAGES.INSTALASI]: { border: 'border-blue-500/50', bg: 'bg-blue-950/40', text: 'text-blue-400' },
  [STAGES.FINISH_INSTALASI]: { border: 'border-sky-500/50', bg: 'bg-sky-950/40', text: 'text-sky-300' },
  [STAGES.BISA_PT1]: { border: 'border-teal-500/50', bg: 'bg-teal-950/40', text: 'text-teal-300' },
  [STAGES.GOLIVE_UT]: { border: 'border-emerald-500/50', bg: 'bg-emerald-950/40', text: 'text-emerald-400' },
};
...
export function StatusDeploymentPanel({ filteredRows = [], activeStage = null, onSelectStage }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 shadow-lg mb-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-slate-100 font-semibold text-lg">Status Deployment (Flow View)</h2>
          <p className="text-xs text-slate-400">Alur progres order dari persiapan ke golive (klik node untuk filter)</p>
        </div>
      </div>
            ...
            return (
              <React.Fragment key={stageName}>
                ...
                >
                  ...
                  <div className="text-xl font-extrabold text-slate-100">{count}</div>
                </button>
      ...
      <div className="mt-5 pt-4 border-t border-slate-800/60">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Sub-Tahap Persiapan</span>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(SUB_STAGES_PERSIAPAN).map(([key, subLabel]) => {
            ...
            return (
              <button
                key={key}
                className="px-3 py-1.5 rounded-full text-xs font-medium border bg-slate-800/40 border-amber-500/30 text-amber-300 hover:bg-amber-500/10 transition-all flex items-center gap-1.5"
                onClick={() => onSelectStage && onSelectStage('subStagePersiapan', subLabel)}
              >
                {subLabel}
                <span className="px-2 py-0.5 rounded bg-slate-900/60 text-[10px] font-bold text-amber-400">
                  {count}
                </span>
              </button>
            );
